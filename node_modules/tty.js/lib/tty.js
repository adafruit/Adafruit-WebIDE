...
function Server(conf, app, server, io) {
  ...
  this.io = io;//io.listen(this.server, {
  //  log: false
  //});
...
Server.prototype.handleConnection = function(socket) {
  ...
  socket.on('create', function(cols, rows, cwd, func) {
    return session.handleCreate(cols, rows, cwd, func);
  });
...
Session.prototype.handleCreate = function(cols, rows, cwd, func) {
  var self = this;
  var terms = this.terms;
  var conf = this.server.conf;
  ...
  term = pty.fork(conf.shell, conf.shellArgs, {
    ...
    cols: cols,
    rows: rows,
    cwd: cwd || conf.cwd || process.env.HOME
  });
